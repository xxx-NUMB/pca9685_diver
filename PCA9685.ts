...
namespace KitiBot {
	...
	*/
    //% blockId=setServo block="Servo |%channel|degree %degree"
    //% weight=85
    //% degree.min=0 degree.max=180
	...
	*/
    //% blockId=setServoPulse block="Servo |%channel|pulse %pulse"
    //% weight=85
    ...
    //% blockId=kitibot_LedRun block="Led|%index|light %light"
    //% light eg: 0
    //% weight=100
	...
	*/
    ...
    export function SensorCalibrated(): void {
        ...
        for (let n = 0; n < 5; n++)  // make the calibration take about 10 seconds
        {
            calibratedMax[n] = 0;
            calibratedMin[n] = 1023;
        }


        for (let o = 0; o < 100; o++)  // make the calibration take about 10 seconds
        {
            if (o < 25 || o >= 75) {
            ...
            for (l = 0; l < 5; l++) {
                let sensor_values2 = AnalogRead();
                for (m = 0; m < 5; m++) {
                    // set the max we found THIS time
                    if ((l == 0) || (max_sensor_values[m] < sensor_values2[m]))
                        max_sensor_values[m] = sensor_values2[m];

                    // set the min we found THIS time
                    if ((l == 0) || (min_sensor_values[m] > sensor_values2[m]))
                        min_sensor_values[m] = sensor_values2[m];
                }
            }

            // record the min and max calibration value
            for (m = 0; m < 5; m++) {
                if (min_sensor_values[m] > calibratedMax[m])
                    calibratedMax[m] = min_sensor_values[m];
                if (max_sensor_values[m] < calibratedMin[m])
                    calibratedMin[m] = max_sensor_values[m];
            }
        }
    ...
    export function readCalibrated(): number[] {
        ...
        let sensor_values3 = AnalogRead();

        for (let p = 0; p < 5; p++) {
            let denominator = calibratedMax[p] - calibratedMin[p];
            let x = ((sensor_values3[p] - calibratedMin[p]) * 1000 / denominator);
            if (x < 0)
                x = 0;
            else if (x > 1000)
                x = 1000;
    ...
    export function readLine(): number {
        ...
        let sensor_values4 = readCalibrated();

        for (q = 0; q < 5; q++) {
            let value = sensor_values4[q];

            if (!white_line)
                value = 1000 - value;
            sensor_values4[q] = value;
